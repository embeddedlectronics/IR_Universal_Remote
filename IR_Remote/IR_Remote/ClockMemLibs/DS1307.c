...
void DS1307_Copy_Date(){
	...
	DS_ReadTime_Delay; 
	// Subtract 1 as the first day of the week  is monday, values are 1 - 7 
	stringcopy(point_time->Display_Date,  Days[(TWI_Read(DS_WeekDay))-1 ]);
	DS_ReadTime_Delay; 
	stringcopy(point_time->Display_Date,  Month[bcdtobin(TWI_Read(DS_Month))-1]);
	DS_ReadTime_Delay; 
...
void DS1307_ReadTime(int address){
	struct DS1307_Properties *point_time = &Time;
	DS_ReadTime_Delay; 
	point_time->CurrentTime = TWI_Read(address);
	
	//_32x16_Draw_Font(x, y, (point_time->CurrentTime>>4)+0x30 , red, green, blue);
	//_32x16_Draw_Font(x+6, y, (point_time->CurrentTime & 0x0F)+0x30 , red, green, blue);	
